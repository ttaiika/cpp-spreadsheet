...
void Cell::Set(std::string text) {
    ...
    if (!temp_ref_cells.empty()) {
        std::unordered_set<const Cell*> ref_collection;
        std::unordered_set<const Cell*> enter_collection;
        std::vector<const Cell*> to_enter_collection;

        for (auto position : temp_ref_cells) {
            ref_collection.insert(dynamic_cast<const Cell*>(sheet_.GetCell(position)));
        }
    ...
    for (const auto& position : impl_->GetReferencedCells()) {
        Cell* refrenced = dynamic_cast<Cell*>(sheet_.GetCell(position));

        if (!refrenced) {
            sheet_.SetCell(position, "");
            refrenced = dynamic_cast<Cell*>(sheet_.GetCell(position));
        }
...
Cell::Value Cell::TextImpl::GetValue() const {
    if (text_.empty()) {
        throw FormulaException("Empty impl");
    }
...
Cell::Value Cell::FormulaImpl::GetValue() const {
    if (!cache_) {
        try {
            ...
            cache_ = e; // сохраняем ошибку в кэше
        }
    }
